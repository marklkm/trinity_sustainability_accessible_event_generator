...
document.getElementById('submitBtn').addEventListener('click', function() {
    ...
    const onlineEvent = document.querySelector('input[name="onlineEvent"]:checked').value;
    const inpersonEvent = document.querySelector('input[name="inpersonEvent"]:checked').value;
    const eventAccess = document.querySelector('input[name="eventAccess"]:checked').value;
    const seatingFacilities = document.querySelector('input[name="seatingFacilities"]:checked').value;
    const wheelchairToilets = document.querySelector('input[name="wheelchairToilets"]:checked').value;
    const hearingLoop = document.querySelector('input[name="hearingLoop"]:checked').value;
    ...
    const sustainableTransport = document.querySelector('input[name="sustainableTransport"]:checked').value;
    const sustainableSourced = document.querySelector('input[name="sustainableSourced"]:checked').value;
    const promoteAwareness = document.querySelector('input[name="promoteAwareness"]:checked').value;
...
document.getElementById('emailBtn').addEventListener('click', function() {
    const output = document.getElementById('output').innerText;
    const mailtoLink = `mailto:?subject=Event Details&body=${encodeURIComponent(output)}`;
...
function copyToTextarea() {
    ...
    formData += "Event Name: " + document.getElementById("eventName").value + "\n";
    formData += "Event Location: " + document.getElementById("eventLocation").value + "\n";
    formData += "Organiser Name: " + document.getElementById("organiserName").value + "\n";
    formData += "Organiser Email: " + document.getElementById("organiserEmail").value + "\n";
    ...
    for (let i = 0; i < seatingFacilitiesEls.length; i++) {
        if (seatingFacilitiesEls[i].checked) {
            formData += "Seating available at the event? " + seatingFacilitiesEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < wheelchairToiletsEls.length; i++) {
        if (seatingFacilitiesEls[i].checked) {
            formData += "Wheelchair accessible toilets? " + wheelchairToiletsEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < accessRequirementsEls.length; i++) {
        if (accessRequirementsEls[i].checked) {
            formData += "Does the event have a question about accessibility requirements of attendees? " + accessRequirementsEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < eventSlidesEls.length; i++) {
        if (eventSlidesEls[i].checked) {
            formData += "Are the presentation slides (MS PowerPoint etc.,.) accessible? " + eventSlidesEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < eventVideoEls.length; i++) {
        if (eventVideoEls[i].checked) {
            formData += "Does the event include a video showing or video call? " + eventVideoEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < eventWasteEls.length; i++) {
        if (eventWasteEls[i].checked) {
            formData += "Will waste be minimised at the event? " + eventWasteEls[i].value + "\n";
            break;
        }
    }

    let sustainableTransportEls = document.getElementsByName("sustainableTransport");
    for (let i = 0; i < sustainableTransportEls.length; i++) {
        if (sustainableTransportEls[i].checked) {
            formData += "Are measures being taken to promote sustainable transportation options? " + sustainableTransportEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < sustainableSourcedEls.length; i++) {
        if (sustainableSourcedEls[i].checked) {
            formData += "Are measures being taken to promote sustainable transportation options? " + sustainableSourcedEls[i].value + "\n";
            break;
        }
    }
    ...
    for (let i = 0; i < promoteAwarenessEls.length; i++) {
        if (promoteAwarenessEls[i].checked) {
            formData += "Will the event promote sustainability awareness and education among attendees? " + promoteAwarenessEls[i].value + "\n";
            break;
        }
    }

    formData += "Any further comments or details? " + document.getElementById("comments").value + "\n";
    formData += "Please provide contact details for any access enquiries (if not included elsewhere): " + document.getElementById("contacts").value + "\n";
